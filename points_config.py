...
# SKUSA SuperNats heat scoring: 0 (P1), 2 (P2), 3 (P3), 4 (P4), ...
def build_skusa_heat_scale(field_size: int = 50) -> Dict[int, int]:
    scale: Dict[int, int] = {1: 0}
    for pos in range(2, field_size + 1):
        scale[pos] = pos  # 2->2, 3->3, 4->4, ...
    return scale

def seed_skusa_sn28(field_size: int = 50, bonus_fast_lap: int = 0, bonus_pole: int = 0) -> None:
    init_db()
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Seed SKUSA SuperNats 28 Heat points")
    ap.add_argument("--field", type=int, default=50, help="Max positions to seed")
    ...
    print(f"Seeded {SCHEME_NAME} Heat points for {args.field} positions")
